...
Camera::Camera(ShaderProgram *shader_program, GLsizei width, GLsizei height)
	:shader_program(shader_program), input(0, 0, -3), 
...
void Camera::update_matrices(){
	proj = glm::perspective(glm::radians(70.0f), (float)(width / height), 0.1f, 500.0f);
	view = glm::mat4(1.0f);
	model = glm::rotate(model, glm::radians(0.5f), glm::vec3(0.53, 0.53f, 0.0f)); // Debug and Test Purpose: should be static (identity Matrix)
