...
namespace Rendering
{
	...
	public:
		...
		void allocateBuffers(unsigned int max_vertex_count, unsigned int max_index_count, 
			unsigned int *indices)
		{
			vao.bind();
			vbo.allocate(max_vertex_count * sizeof(Geometry::Vertex));
			ibo.bufferData(indices,
				max_index_count * sizeof(unsigned int));
		}
		/* Push the batched data to the Vertex Buffer */
		void bufferBatch(const Mesh& mesh, unsigned int offset)
		{
			vao.bind();
		...
		*/
		void bindLayout()
		{
			...
			vao.linkAttrib(&vbo, 1, 3, 
				GL_FLOAT, sizeof(Geometry::Vertex),
				offsetof(Geometry::Vertex, tex_coords));
			vao.linkAttrib(&vbo, 2, 1, 
				GL_FLOAT, sizeof(Geometry::Vertex),
